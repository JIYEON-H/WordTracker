...
Generated By - Jiyeon Heo, Tze-chi Chan, and Parinthorn Songsana
...
Installation & Usage
1. Check your JDK version, using 'java -version', or 'javac -version' on terminal. It should be higher than 1.8.0_202.
2. If you cannot check any on terminal. You should download it first to use this program.
3. Open terminal, compile the Java Jar file by following command.
...
Limitations
-This program designed for tracking words with file name and line numbers.
...
-Provide absolute path if the program cannot find the file name.
-An optional argument is (-f output.txt), include space between '-f' and output file name.
-The repository.ser and the output files will be stored in 'res' folder.
-The passed file names are stored in 'res/processedFiles.txt', which is prevent duplicate data.
